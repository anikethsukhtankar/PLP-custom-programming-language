...
public class Scanner {
	...
	public class Token {
		 ...
		 * and length.
		 * 
		 * This method, along with hash code, was generated by eclipse.
		 */
		@Override
	...
	Scanner(String inputString) {
	 ...
	 private enum State {START,COMMENT,COMMENTCLOSE,HAS_ZERO,IN_DIGIT,IN_IDENT,IN_FLOAT,HAS_EQ,HAS_GT,HAS_LT,IN_FLOAT_DOT,HAS_COLON,HAS_EXCLAMATION,HAS_TIMES,HAS_DIV};  
			...
			switch(state) {
				case START: {
					...
					switch (ch) {
						...
						case '$':{error(pos, line(pos), posInLine(pos), "$ can only appear in the middle of an identifier");} break;
						case '_':{error(pos, line(pos), posInLine(pos), "_ can only appear in the middle of an identifier");} break;
						
						case '=':{state = State.HAS_EQ; pos++;} break;
						case '>':{state = State.HAS_GT; pos++;} break;
						case '<':{state = State.HAS_LT; pos++;} break;
						case '.':{state = State.IN_FLOAT_DOT; pos++;} break;
						case ':':{state = State.HAS_COLON; pos++;} break;
						case '!':{state = State.HAS_EXCLAMATION; pos++;} break;
						case '*':{state = State.HAS_TIMES; pos++;} break;
						case '/':{state = State.HAS_DIV; pos++;} break;
						case '0':{state = State.HAS_ZERO; pos++;} break;
				...
				case COMMENT: {
					if(ch == '*'){
						state = State.COMMENTCLOSE;
						pos++;
					}
					else if(ch == EOFChar) {
						error(pos, line(pos), posInLine(pos), "unexpected EOF in comment at");
					}
					else if(ch < 0 || ch >= 128){
							error(pos, line(pos), posInLine(pos), "illegal char inside comments");	
					}
					else {
						pos++;
					}
				...
				case HAS_EQ: {
					if(ch != EOFChar && ch == '=') {
						...
						error(pos, line(pos), posInLine(pos), "= expected at");
					}
				} break;
				case IN_DIGIT: {
					if(ch != EOFChar && ch == '.') {
					...
					else if(ch != EOFChar && Character.isDigit(ch)) {
						pos++;
					}
					else {
						String int_str = new String(chars, startPos, pos - startPos);
						try {
						    Integer.parseInt(int_str);
						} catch (NumberFormatException e) {
							error(startPos, line(startPos), posInLine(startPos), "Integer is out of bounds");
						}
				...
				case IN_FLOAT: {
					if(ch != EOFChar && Character.isDigit(ch)) {
						...
						String float_str = new String(chars, startPos, pos - startPos);
						if(Float.isFinite(Float.valueOf(float_str))) {
							tokens.add(new Token(Kind.FLOAT_LITERAL, startPos, pos - startPos));
							state = State.START;
						}
						else {
							error(startPos, line(startPos), posInLine(startPos), "Float is out of bounds");
						}	
					}
				} break;
				case IN_FLOAT_DOT: {
				...
				case IN_IDENT: {
					if(ch != EOFChar && ((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || Character.isDigit(ch) || ch == '_' || ch == '$')) {
						pos++;
					}
					else {
						String ident_str = new String(chars, startPos, pos - startPos);
						switch (ident_str) {
							case "Z":{tokens.add(new Token(Kind.KW_Z, startPos, pos - startPos)); state = State.START;} break;
							case "default_width":{tokens.add(new Token(Kind.KW_default_width, startPos, pos - startPos)); state = State.START;} break;
							case "default_height":{tokens.add(new Token(Kind.KW_default_height, startPos, pos - startPos)); state = State.START;} break;
							case "show":{tokens.add(new Token(Kind.KW_show, startPos, pos - startPos)); state = State.START;} break;
							case "write":{tokens.add(new Token(Kind.KW_write, startPos, pos - startPos)); state = State.START;} break;
							case "to":{tokens.add(new Token(Kind.KW_to, startPos, pos - startPos)); state = State.START;} break;
							case "input":{tokens.add(new Token(Kind.KW_input, startPos, pos - startPos)); state = State.START;} break;
							case "from":{tokens.add(new Token(Kind.KW_from, startPos, pos - startPos)); state = State.START;} break;
							case "cart_x":{tokens.add(new Token(Kind.KW_cart_x, startPos, pos - startPos)); state = State.START;} break;
							case "cart_y":{tokens.add(new Token(Kind.KW_cart_y, startPos, pos - startPos)); state = State.START;} break;
							case "polar_a":{tokens.add(new Token(Kind.KW_polar_a, startPos, pos - startPos)); state = State.START;} break;
							case "polar_r":{tokens.add(new Token(Kind.KW_polar_r, startPos, pos - startPos)); state = State.START;} break;
							case "abs":{tokens.add(new Token(Kind.KW_abs, startPos, pos - startPos)); state = State.START;} break;
							case "sin":{tokens.add(new Token(Kind.KW_sin, startPos, pos - startPos)); state = State.START;} break;
							case "cos":{tokens.add(new Token(Kind.KW_cos, startPos, pos - startPos)); state = State.START;} break;
							case "atan":{tokens.add(new Token(Kind.KW_atan, startPos, pos - startPos)); state = State.START;} break;
							case "log":{tokens.add(new Token(Kind.KW_log, startPos, pos - startPos)); state = State.START;} break;
							case "image":{tokens.add(new Token(Kind.KW_image, startPos, pos - startPos)); state = State.START;} break;
							case "int":{tokens.add(new Token(Kind.KW_int, startPos, pos - startPos)); state = State.START;} break;
							case "float":{tokens.add(new Token(Kind.KW_float, startPos, pos - startPos)); state = State.START;} break;
							case "filename":{tokens.add(new Token(Kind.KW_filename, startPos, pos - startPos)); state = State.START;} break;
							case "boolean":{tokens.add(new Token(Kind.KW_boolean, startPos, pos - startPos)); state = State.START;} break;
							case "red":{tokens.add(new Token(Kind.KW_red, startPos, pos - startPos)); state = State.START;} break;
							case "blue":{tokens.add(new Token(Kind.KW_blue, startPos, pos - startPos)); state = State.START;} break;
							case "green":{tokens.add(new Token(Kind.KW_green, startPos, pos - startPos)); state = State.START;} break;
							case "alpha":{tokens.add(new Token(Kind.KW_alpha, startPos, pos - startPos)); state = State.START;} break;
							case "while":{tokens.add(new Token(Kind.KW_while, startPos, pos - startPos)); state = State.START;} break;
							case "if":{tokens.add(new Token(Kind.KW_if, startPos, pos - startPos)); state = State.START;} break;
							case "width":{tokens.add(new Token(Kind.KW_width, startPos, pos - startPos)); state = State.START;} break;
							case "height":{tokens.add(new Token(Kind.KW_height, startPos, pos - startPos)); state = State.START;} break;
							case "sleep":{tokens.add(new Token(Kind.KW_sleep, startPos, pos - startPos)); state = State.START;} break;
							case "true":{tokens.add(new Token(Kind.BOOLEAN_LITERAL, startPos, pos - startPos)); state = State.START;} break;
							case "false":{tokens.add(new Token(Kind.BOOLEAN_LITERAL, startPos, pos - startPos)); state = State.START;} break;
							default: {
